...
#include "PreProcessorDirectives.hxx"
...
std::ostream &operator<<(std::ostream &os, AndOrSearchSpace::SearchAndNode_WithPath &n) { 
    os << "AND_NODE: (";
	os << "Var: " << n.V() << ", ";
	os << "Val: " << n.Value() << ", ";
	os << "h: " << n.h() << ", ";
	os << "w: " << n.ISw() << ", ";
	os << "q: " << n.ISq() << ", ";
	os << "Path: " << ARE::print( n.PathAssignment() ) << ", ";
	os << "RelCtxt: " << ARE::print( n.WS()->MapVar2AbstractionContext(n.V()) );
    os << ")";
	return os;
}
...
//	std::vector<double> qLIST ; qLIST.reserve(n) ; qLIST.resize(n) ;
	if (n > 1) {
		...
		// compute the sum of q's
			...
			std::cout << "\t\t\t\t" << "Abstract State: " << std::endl;
		#endif //LOG_PROBES
		for (k = idxS ; k < idxE ; ++k) {
...
//			qSUM += v ;
		...
		#ifdef LOG_PROBES
			std::cout << "\t\t\t\t" << "Selected Representative: " << std::endl;
			std::cout << "\t\t\t\t\t" << *anRes << std::endl;
		#endif //LOG_PROBES
		}
	else {
		anRes = openList[idxS] ;
...
int32_t AbsSamplingTwoAndNodeCompare_RevHeuristicSimple(void *Obj1, void *Obj2)
{
	if (Obj1 == Obj2) 
		return 0 ;
...
int32_t AbsSamplingTwoAndNodeCompare_HRSimple(void *Obj1, void *Obj2)
{
	if (Obj1 == Obj2) 
		return 0 ;
	...
	double hr1 = A1->h() + A1->QPath();
	double hr2 = A2->h() + A2->QPath();
...
int32_t AbsSamplingTwoAndNodeCompare_RevHRSimple(void *Obj1, void *Obj2)
{
	if (Obj1 == Obj2) 
		return 0 ;
...
int32_t AbsSamplingTwoAndNodeCompare_QSimple(void *Obj1, void *Obj2)
{
	if (Obj1 == Obj2) 
		return 0 ;
...
int32_t AbsSamplingTwoAndNodeCompare_RevQSimple(void *Obj1, void *Obj2)
{
	if (Obj1 == Obj2) 
		return 0 ;
...
double getHR(void *Obj){ 
	AndOrSearchSpace::SearchAndNode_WithPath *n = (AndOrSearchSpace::SearchAndNode_WithPath*) Obj ;
	return n->h() + n->QPath(); 
	}

double getQ(void *Obj){ 
	AndOrSearchSpace::SearchAndNode_WithPath *n = (AndOrSearchSpace::SearchAndNode_WithPath*) Obj ;
	return n->ISq(); 
	}
...
void AccumulateNodeLogHRValue(double& accumulatedValue, void *Obj1)
// we assume all AND nodes are instances of SearchAndNode_WithPath class.
// we will compare the two nodes along the path from Obj1/Obj2 to the root; 
// some nodes on the path may not be in the context.
// we also assume that '_PathAssignment' member variable of a1/a2 is full path assigment, from a1/a2 to the root, not from a1/a2 to the closest branching variable.
{
	AndOrSearchSpace::SearchAndNode_WithPath *n = (AndOrSearchSpace::SearchAndNode_WithPath*) Obj1 ;
	LOG_OF_SUM_OF_TWO_NUMBERS_GIVEN_AS_LOGS(accumulatedValue, accumulatedValue, ( n->h() + n->QPath() ) )
}

void AccumulateNodeLogQValue(double& accumulatedValue, void *Obj1)
// we assume all AND nodes are instances of SearchAndNode_WithPath class.
// we will compare the two nodes along the path from Obj1/Obj2 to the root; 
// some nodes on the path may not be in the context.
// we also assume that '_PathAssignment' member variable of a1/a2 is full path assigment, from a1/a2 to the root, not from a1/a2 to the closest branching variable.
{
	AndOrSearchSpace::SearchAndNode_WithPath *n = (AndOrSearchSpace::SearchAndNode_WithPath*) Obj1 ;
	LOG_OF_SUM_OF_TWO_NUMBERS_GIVEN_AS_LOGS(accumulatedValue, accumulatedValue, n->ISq())
}
...
const std::unordered_map<std::string, AS_FXNS> AS_FXNS_MAP{
		...
		{"simpleRand_dfs", simpleRand_dfs},
		{"simpleRandRand_dfs", simpleRandRand_dfs},
		{"simpleHB_dfs", simpleHB_dfs},
		{"equalDistHB_dfs", balancedHB_dfs},
		{"equalDistHB2_dfs", balancedHB2_dfs},
		{"equalDistHB3_dfs", balancedHB3_dfs},
		{"equalDistHB4_dfs", balancedHB4_dfs},
		{"minVarHB_dfs", minVarHB_dfs},
		{"randHB_dfs", randHB_dfs},
		{"simpleHRB_dfs", simpleHRB_dfs},
		{"equalDistHRB_dfs", balancedHRB_dfs},
		{"equalDistHRB2_dfs", balancedHRB2_dfs},
		{"equalDistHRB3_dfs", balancedHRB3_dfs},
		{"equalDistHRB4_dfs", balancedHRB4_dfs},
		{"minVarHRB_dfs", minVarHRB_dfs},
		{"randHRB_dfs", randHRB_dfs},
		{"simpleQB_dfs", simpleQB_dfs},
		{"equalDistQB_dfs", balancedQB_dfs},
		{"equalDistQB2_dfs", balancedQB2_dfs},
		{"equalDistQB3_dfs", balancedQB3_dfs},
		{"equalDistQB4_dfs", balancedQB4_dfs},
		{"minVarQB_dfs", minVarQB_dfs},
		{"randQB_dfs", randQB_dfs},
	} ;

const std::unordered_map<AS_FXNS, AbsSamplingCompFn*> AS_NODE_COMP_FNS{
		{unique, &AbsSamplingTwoAndNodeCompare_Unique},
		{customproper_dfs, &AbsSamplingTwoAndNodeCompare_CustomProper_DFS},
		{customproper_bfs, &AbsSamplingTwoAndNodeCompare_CustomProper},
		{relCB_dfs, &AbsSamplingTwoAndNodeCompare_CustomProper_DFS},
		{relCB_bfs, &AbsSamplingTwoAndNodeCompare_CustomProper},
		{randCB_dfs, &AbsSamplingTwoAndNodeCompare_RandCntxt},
		{simpleRand_dfs, &AbsSamplingTwoAndNodeCompare_HeuristicSimple},
		{simpleRandRand_dfs, &AbsSamplingTwoAndNodeCompare_HeuristicSimple},
		{simpleHB_dfs, &AbsSamplingTwoAndNodeCompare_HeuristicSimple},
		{balancedHB_dfs, &AbsSamplingTwoAndNodeCompare_HeuristicSimple},
		{balancedHB2_dfs, &AbsSamplingTwoAndNodeCompare_RevHeuristicSimple},
		{balancedHB3_dfs, &AbsSamplingTwoAndNodeCompare_RevHeuristicSimple},
		{balancedHB4_dfs, &AbsSamplingTwoAndNodeCompare_RevHeuristicSimple},
		{minVarHB_dfs, &AbsSamplingTwoAndNodeCompare_RevHeuristicSimple},
		{randHB_dfs, &AbsSamplingTwoAndNodeCompare_RevHeuristicSimple},
		{simpleHRB_dfs, &AbsSamplingTwoAndNodeCompare_HRSimple},
		{balancedHRB_dfs, &AbsSamplingTwoAndNodeCompare_HRSimple},
		{balancedHRB2_dfs, &AbsSamplingTwoAndNodeCompare_RevHRSimple},
		{balancedHRB3_dfs, &AbsSamplingTwoAndNodeCompare_RevHRSimple},
		{balancedHRB4_dfs, &AbsSamplingTwoAndNodeCompare_RevHRSimple},
		{minVarHRB_dfs, &AbsSamplingTwoAndNodeCompare_RevHRSimple},
		{randHRB_dfs, &AbsSamplingTwoAndNodeCompare_RevHRSimple},
		{simpleQB_dfs, &AbsSamplingTwoAndNodeCompare_QSimple},
		{balancedQB_dfs, &AbsSamplingTwoAndNodeCompare_QSimple},
		{balancedQB2_dfs, &AbsSamplingTwoAndNodeCompare_RevQSimple},
		{balancedQB3_dfs, &AbsSamplingTwoAndNodeCompare_RevQSimple},
		{balancedQB4_dfs, &AbsSamplingTwoAndNodeCompare_RevQSimple},
		{minVarQB_dfs, &AbsSamplingTwoAndNodeCompare_RevQSimple},
		{randQB_dfs, &AbsSamplingTwoAndNodeCompare_RevQSimple},
	} ;
// AbsSamplingCompFn* AS_NODE_COMP_FNS[10] {
...
// 		&AbsSamplingTwoAndNodeCompare_HeuristicSimple, //HBEqualDist2
// 		&AbsSamplingTwoAndNodeCompare_HeuristicSimple //minVarHB_dfs
// 	} ;


const std::unordered_map<AS_FXNS, AbsSamplingAccumFn*> AS_ACCUM_FNS{
		{unique, nullptr},
		{customproper_dfs, nullptr},
		{customproper_bfs, nullptr},
		{relCB_dfs, nullptr},
		{relCB_bfs, nullptr},
		{randCB_dfs, nullptr},
		{simpleRand_dfs, nullptr},
		{simpleRandRand_dfs, nullptr},
		{simpleHB_dfs, nullptr},
		{balancedHB_dfs, &AccumulateNodeLogHeuristicValue},
		{balancedHB2_dfs, &AccumulateNodeLogHeuristicValue},
		{balancedHB3_dfs, &AccumulateNodeLogHeuristicValue},
		{balancedHB4_dfs, &AccumulateNodeLogHeuristicValue},
		{minVarHB_dfs, nullptr},
		{randHB_dfs, nullptr},
		{simpleHRB_dfs, nullptr},
		{balancedHRB_dfs, &AccumulateNodeLogHRValue},
		{balancedHRB2_dfs, &AccumulateNodeLogHRValue},
		{balancedHRB3_dfs, &AccumulateNodeLogHRValue},
		{balancedHRB4_dfs, &AccumulateNodeLogHRValue},
		{minVarHRB_dfs, nullptr},
		{randHRB_dfs, nullptr},
		{simpleQB_dfs, nullptr},
		{balancedQB_dfs, &AccumulateNodeLogQValue},
		{balancedQB2_dfs, &AccumulateNodeLogQValue},
		{balancedQB3_dfs, &AccumulateNodeLogQValue},
		{balancedQB4_dfs, &AccumulateNodeLogQValue},
		{minVarQB_dfs, nullptr},
		{randQB_dfs, nullptr},
	} ;
// AbsSamplingAccumFn* AS_ACCUM_FNS[10] {
// 		nullptr, //unique
...
// 		&AccumulateNodeLogHeuristicValue, //HBEqualDist2
// 		nullptr, //minVarHB_dfs
// 	} ;


const std::unordered_map<AS_FXNS, AbsSamplingNodeValueExtractionFn*> AS_NODE_VALUE_FNS{
		{unique, nullptr},
		{customproper_dfs, nullptr},
		{customproper_bfs, nullptr},
		{relCB_dfs, nullptr},
		...
		{simpleRand_dfs, nullptr},
		{simpleRandRand_dfs, nullptr},
		{simpleHB_dfs, &getHeur},
		{balancedHB_dfs, &getHeur},
		{balancedHB2_dfs, &getHeur},
		{balancedHB3_dfs, &getHeur},
		{balancedHB4_dfs, &getHeur},
		{minVarHB_dfs, &getHeur},
		{randHB_dfs, &getHeur},
		{simpleHRB_dfs, &getHR},
		{balancedHRB_dfs, &getHR},
		{balancedHRB2_dfs, &getHR},
		{balancedHRB3_dfs, &getHR},
		{balancedHRB4_dfs, &getHR},
		{minVarHRB_dfs, &getHR},
		{randHRB_dfs, &getHR},
		{simpleQB_dfs, &getQ},
		{balancedQB_dfs, &getQ},
		{balancedQB2_dfs, &getQ},
		{balancedQB3_dfs, &getQ},
		{balancedQB4_dfs, &getQ},
		{minVarQB_dfs, &getQ},
		{randQB_dfs, &getQ},
	} ;

// enum AS_ALG_PROPERTY_IDX {dfs=0, randomized, dynamic} ;
const std::unordered_map<AS_FXNS, std::array<bool, 3>> AS_ALG_PROPERTIES{
		{unique, std::array<bool, 3>{true, false, false}},
		{customproper_dfs, std::array<bool, 3>{true, false, false}},
		{customproper_bfs, std::array<bool, 3>{false, false, false}},
		{relCB_dfs, std::array<bool, 3>{true, false, false}},
		{relCB_bfs, std::array<bool, 3>{false, false, false}},
		{randCB_dfs, std::array<bool, 3>{true, true, false}},
		{simpleRand_dfs, std::array<bool, 3>{true, true, false}},
		{simpleRandRand_dfs, std::array<bool, 3>{true, true, false}},
		{simpleHB_dfs, std::array<bool, 3>{true, false, true}},
		{balancedHB_dfs, std::array<bool, 3>{true, false, true}},
		{balancedHB2_dfs, std::array<bool, 3>{true, false, true}},
		{balancedHB3_dfs, std::array<bool, 3>{true, false, true}},
		{balancedHB4_dfs, std::array<bool, 3>{true, false, true}},
		{minVarHB_dfs, std::array<bool, 3>{true, false, true}},
		{randHB_dfs, std::array<bool, 3>{true, true, true}},
		{simpleHRB_dfs, std::array<bool, 3>{true, false, true}},
		{balancedHRB_dfs, std::array<bool, 3>{true, false, true}},
		{balancedHRB2_dfs, std::array<bool, 3>{true, false, true}},
		{balancedHRB3_dfs, std::array<bool, 3>{true, false, true}},
		{balancedHRB4_dfs, std::array<bool, 3>{true, false, true}},
		{minVarHRB_dfs, std::array<bool, 3>{true, false, true}},
		{randHRB_dfs, std::array<bool, 3>{true, true, true}},
		{simpleQB_dfs, std::array<bool, 3>{true, false, true}},
		{balancedQB_dfs, std::array<bool, 3>{true, false, true}},
		{balancedQB2_dfs, std::array<bool, 3>{true, false, true}},
		{balancedQB3_dfs, std::array<bool, 3>{true, false, true}},
		{balancedQB4_dfs, std::array<bool, 3>{true, false, true}},
		{minVarQB_dfs, std::array<bool, 3>{true, false, true}},
		{randQB_dfs, std::array<bool, 3>{true, true, true}},
	} ;
// // enum AS_ALG_PROPERTY_IDX {dfs=0, randomized, dynamic} ;
// bool AS_ALG_PROPERTIES[10][3] {
