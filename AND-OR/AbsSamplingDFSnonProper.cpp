...
#ifdef ABS_SAMPLING_DFS_NON_PROPER_ASSERTIONS
#include <cassert>
#endif
...
// extern MTRand RNG_abs_cntxt_gen;
...
*/
	...
	while (NULL != dfs_current) {
		...
		#ifdef LOG_PROBES
			std::cout << "\t" << "New Branch Root:" << std::endl;
		#endif //LOG_PROBES
...
expand_more :
		...
		#ifdef LOG_PROBES
			std::cout << "\t\t" << "Var: " << vC << std::endl;
			std::cout << "\t\t\t" << "Par: " << (bParent == NULL ? -1 : bParent->V()) << std::endl;
		#endif //LOG_PROBES
		...
		//TODO: remove conditionals by refactoring into functions that can be selected upon AbstractionSamplingWorkspace construction
			...
			// heuristic-based sorting
			else if (_CompFn == AbsSamplingTwoAndNodeCompare_HeuristicSimple || _CompFn == AbsSamplingTwoAndNodeCompare_RevHeuristicSimple){
				if(_Alg.abs_fxn==simpleHB_dfs) {
					...
					if (_nLevelsLimit >= 0 && _NumberOfAncestorBranchingVariables[temp_n->V()] >= _nLevelsLimit) {
						...
						int num_abs_states_minus_one = num_abs_states-1;
						int32_t idxS = 0, idxE = -1 ;
						for(int abs_state = 0; abs_state < num_abs_states; ++abs_state){
							if(nAbsCreated == num_abs_states_minus_one){
								SearchAndNode_WithPath *an_merged = DoISmerge(OL_to, idxS, OL_to.size()) ;
								OL_from.push_back(an_merged) ;
								++nAbsCreated;
								break;
								}
						...
						#ifdef ABS_SAMPLING_DFS_NON_PROPER_ASSERTIONS
							assert(nAbsCreated <= _nRandAbs);
						#endif
						}
					}
				else if(_Alg.abs_fxn==simpleRand_dfs) {
					//NOTE: currently assumes nodes are from the same variable since DFS
					//do a Knuth abstraction if reached _nLevelsLimit
					AndOrSearchSpace::SearchAndNode *temp_n = (AndOrSearchSpace::SearchAndNode*) OL_to[0] ;
					if (_nLevelsLimit >= 0 && _NumberOfAncestorBranchingVariables[temp_n->V()] >= _nLevelsLimit) {
						...
						if (OL_to.size() > 1) 
							std::shuffle(OL_to.begin(), OL_to.end(), _rng) ;
						int floor_node_count_per_abs_state = OL_to.size() / _nRandAbs;
						int num_states_remaining_needing_extra_member = OL_to.size() % _nRandAbs;
						int num_abs_states = floor_node_count_per_abs_state == 0 ? OL_to.size() : _nRandAbs ;
						int num_abs_states_minus_one = num_abs_states - 1;
				...
				else if(_Alg.abs_fxn==simpleRandRand_dfs) {
					//NOTE: currently assumes nodes are from the same variable since DFS
					//do a Knuth abstraction if reached _nLevelsLimit
					AndOrSearchSpace::SearchAndNode *temp_n = (AndOrSearchSpace::SearchAndNode*) OL_to[0] ;
					if (_nLevelsLimit >= 0 && _NumberOfAncestorBranchingVariables[temp_n->V()] >= _nLevelsLimit) {
					...
					if (OL_to.size() <= _nRandAbs){
						for(uint32_t i = 0; i < OL_to.size(); ++i){
							OL_from.push_back(OL_to[i]) ;
						}
					}
					else {
						if (OL_to.size() > 1){
						...
						int maxIdx = OL_to.size()-1;
						std::vector<int32_t> ivec(maxIdx);
						std::iota(ivec.begin(), ivec.end(), 1); // ivec will become: [1..OL_to.size()-1]
						std::shuffle(ivec.begin(), ivec.end(), _rng);
						ivec.resize(_nRandAbs);
						ivec[_nRandAbs-1] = OL_to.size(); //ivec now holds all random generated split points (exclusive endpoint of partitions) + OL_to_size()
						std::sort(ivec.begin(), ivec.end());
						int num_abs_states_minus_one = _nRandAbs - 1;
						int32_t idxS = 0, idxE = -1 ;
						for(int i = 0; i < _nRandAbs; ++i){
							if(nAbsCreated == num_abs_states_minus_one){
								SearchAndNode_WithPath *an_merged = DoISmerge(OL_to, idxS, OL_to.size()) ;
								OL_from.push_back(an_merged) ;
								++nAbsCreated;
								...
								SearchAndNode_WithPath *an_merged = DoISmerge(OL_to, idxS, idxE) ;
								OL_from.push_back(an_merged) ;
								idxS = idxE ; //idx is already incremented past last element
								++nAbsCreated;
								}
							}
						#ifdef ABS_SAMPLING_DFS_NON_PROPER_ASSERTIONS
				...
				else if(_Alg.abs_fxn==balancedHB2_dfs){
					//NOTE: currently assumes nodes are from the same variable since DFS
					//do a Knuth abstraction if reached _nLevelsLimit
					AndOrSearchSpace::SearchAndNode *temp_n = (AndOrSearchSpace::SearchAndNode*) OL_to[0] ;
					if (_nLevelsLimit >= 0 && _NumberOfAncestorBranchingVariables[temp_n->V()] >= _nLevelsLimit) {
				...
				else if(_Alg.abs_fxn==balancedHB3_dfs){
					//NOTE: currently assumes nodes are from the same variable since DFS
					//do a Knuth abstraction if reached _nLevelsLimit
					AndOrSearchSpace::SearchAndNode *temp_n = (AndOrSearchSpace::SearchAndNode*) OL_to[0] ;
					if (_nLevelsLimit >= 0 && _NumberOfAncestorBranchingVariables[temp_n->V()] >= _nLevelsLimit) {
				...
				else if(_Alg.abs_fxn==balancedHB4_dfs){
					//NOTE: currently assumes nodes are from the same variable since DFS
					//do a Knuth abstraction if reached _nLevelsLimit
					AndOrSearchSpace::SearchAndNode *temp_n = (AndOrSearchSpace::SearchAndNode*) OL_to[0] ;
					if (_nLevelsLimit >= 0 && _NumberOfAncestorBranchingVariables[temp_n->V()] >= _nLevelsLimit) {
						...
						if (totalAccumlationOfAllH == -INFINITY) {
							...
							int numPartitions = _nRandAbs;
							double totalRemainingAccumulationofH = totalAccumlationOfAllH;
							double intervalH = totalRemainingAccumulationofH - log10(numPartitions);
							double intervalRunningAccumulation =  -INFINITY;
							int num_abs_states_minus_one = _nRandAbs - 1;
							int32_t idxS = 0, idxE = -1 ;
							while(idxS < OL_to.size()){
								if(nAbsCreated == num_abs_states_minus_one){
									...
									int32_t idx = idxS;
									do{
										LOG_OF_SUM_OF_TWO_NUMBERS_GIVEN_AS_LOGS(intervalRunningAccumulation, intervalRunningAccumulation, OL_to[idx]->h())
										++idx;
										if(idx == OL_to.size()){
											break;
											}
										} while(intervalRunningAccumulation < intervalH);
									idxE = idx; //idx already incremented to past old index accessed above
									SearchAndNode_WithPath *an_merged = DoISmerge(OL_to, idxS, idxE) ;
									OL_from.push_back(an_merged) ;
									idxS = idxE ;
									++nAbsCreated;
									numPartitions -= 1;
									// compute next interval end based on remaining mass and remaining abstract states
									LOG_OF_SUB_OF_TWO_NUMBERS_GIVEN_AS_LOGS(totalRemainingAccumulationofH, totalRemainingAccumulationofH, intervalRunningAccumulation) 
									intervalH = totalRemainingAccumulationofH - log10(numPartitions);
									intervalRunningAccumulation =  -INFINITY;
									}
								}
							#ifdef ABS_SAMPLING_DFS_NON_PROPER_ASSERTIONS
								assert(nAbsCreated <= _nRandAbs);
				...
				else if(_Alg.abs_fxn==randHB_dfs){
					//NOTE: currently assumes nodes are from the same variable since DFS
					//do a Knuth abstraction if reached _nLevelsLimit
					AndOrSearchSpace::SearchAndNode *temp_n = (AndOrSearchSpace::SearchAndNode*) OL_to[0] ;
					if (_nLevelsLimit >= 0 && _NumberOfAncestorBranchingVariables[temp_n->V()] >= _nLevelsLimit) {
...
////////////
			...
			else if (_CompFn == AbsSamplingTwoAndNodeCompare_HRSimple || _CompFn == AbsSamplingTwoAndNodeCompare_RevHRSimple){
				if(_Alg.abs_fxn==simpleHRB_dfs) {
					//NOTE: currently assumes nodes are from the same variable since DFS
					//do a Knuth abstraction if reached _nLevelsLimit
					AndOrSearchSpace::SearchAndNode *temp_n = (AndOrSearchSpace::SearchAndNode*) OL_to[0] ;
					if (_nLevelsLimit >= 0 && _NumberOfAncestorBranchingVariables[temp_n->V()] >= _nLevelsLimit) {
				...
				else if(_Alg.abs_fxn==balancedHRB_dfs){
					//NOTE: currently assumes nodes are from the same variable since DFS
					//do a Knuth abstraction if reached _nLevelsLimit
					AndOrSearchSpace::SearchAndNode *temp_n = (AndOrSearchSpace::SearchAndNode*) OL_to[0] ;
					if (_nLevelsLimit >= 0 && _NumberOfAncestorBranchingVariables[temp_n->V()] >= _nLevelsLimit) {
						...
						double totalAccumlationOfAllHR = -INFINITY;
						if (OL_to.size() > 1) {
							totalAccumlationOfAllHR = QuickSortAccumulated((void**) OL_to.data(), OL_to.size(), left, right, _CompFn, _AccumFn, -INFINITY);
							}
						else{
							totalAccumlationOfAllHR = ( OL_to[0]->h() + OL_to[0]->QPath() );
							}
						if (totalAccumlationOfAllHR == -INFINITY) {
							SearchAndNode_WithPath *an_merged = DoISmerge(OL_to, 0, OL_to.size()-1) ;
							OL_from.push_back(an_merged) ;
							}
						else{
							double intervalHR = totalAccumlationOfAllHR - log10(_nRandAbs);
							double currentIntervalEnd = intervalHR;
							double currentRunningAccumulation =  -INFINITY;
							int num_abs_states_minus_one = _nRandAbs - 1;
							int32_t idxS = 0, idxE = -1 ;
							while(idxS < OL_to.size()){
								if(nAbsCreated == num_abs_states_minus_one){
									...
									int32_t idx = idxS;
									while(currentRunningAccumulation < currentIntervalEnd){
										LOG_OF_SUM_OF_TWO_NUMBERS_GIVEN_AS_LOGS(currentRunningAccumulation, currentRunningAccumulation, ( OL_to[idx]->h() + OL_to[idx]->QPath() ))
										++idx;
										if(idx == OL_to.size()){
											break;
											}
									...
									++nAbsCreated;
									while(currentIntervalEnd <= currentRunningAccumulation){
										LOG_OF_SUM_OF_TWO_NUMBERS_GIVEN_AS_LOGS(currentIntervalEnd, currentIntervalEnd, intervalHR);
										}
									}
								}
							#ifdef ABS_SAMPLING_DFS_NON_PROPER_ASSERTIONS
				...
				else if(_Alg.abs_fxn==balancedHRB2_dfs){
					//NOTE: currently assumes nodes are from the same variable since DFS
					//do a Knuth abstraction if reached _nLevelsLimit
					AndOrSearchSpace::SearchAndNode *temp_n = (AndOrSearchSpace::SearchAndNode*) OL_to[0] ;
					if (_nLevelsLimit >= 0 && _NumberOfAncestorBranchingVariables[temp_n->V()] >= _nLevelsLimit) {
				...
				else if(_Alg.abs_fxn==balancedHRB3_dfs){
					//NOTE: currently assumes nodes are from the same variable since DFS
					//do a Knuth abstraction if reached _nLevelsLimit
					AndOrSearchSpace::SearchAndNode *temp_n = (AndOrSearchSpace::SearchAndNode*) OL_to[0] ;
					if (_nLevelsLimit >= 0 && _NumberOfAncestorBranchingVariables[temp_n->V()] >= _nLevelsLimit) {
				...
				else if(_Alg.abs_fxn==balancedHRB4_dfs){
					//NOTE: currently assumes nodes are from the same variable since DFS
					//do a Knuth abstraction if reached _nLevelsLimit
					AndOrSearchSpace::SearchAndNode *temp_n = (AndOrSearchSpace::SearchAndNode*) OL_to[0] ;
					if (_nLevelsLimit >= 0 && _NumberOfAncestorBranchingVariables[temp_n->V()] >= _nLevelsLimit) {
						...
						if (totalAccumlationOfAllHR == -INFINITY) {
							...
							int numPartitions = _nRandAbs;
							double totalRemainingAccumulationofHR = totalAccumlationOfAllHR;
							double intervalHR = totalRemainingAccumulationofHR - log10(numPartitions);
							double intervalRunningAccumulation =  -INFINITY;
							int num_abs_states_minus_one = _nRandAbs - 1;
							int32_t idxS = 0, idxE = -1 ;
							while(idxS < OL_to.size()){
								if(nAbsCreated == num_abs_states_minus_one){
									...
									int32_t idx = idxS;
									do{
										LOG_OF_SUM_OF_TWO_NUMBERS_GIVEN_AS_LOGS(intervalRunningAccumulation, intervalRunningAccumulation, ( OL_to[idx]->h() + OL_to[idx]->QPath() ))
										++idx;
										if(idx == OL_to.size()){
											break;
											}
										} while(intervalRunningAccumulation < intervalHR);
									idxE = idx; //idx already incremented to past old index accessed above
									SearchAndNode_WithPath *an_merged = DoISmerge(OL_to, idxS, idxE) ;
									OL_from.push_back(an_merged) ;
									idxS = idxE ;
									...
									numPartitions -= 1;
									// compute next interval end based on remaining mass and remaining abstract states
									LOG_OF_SUB_OF_TWO_NUMBERS_GIVEN_AS_LOGS(totalRemainingAccumulationofHR, totalRemainingAccumulationofHR, intervalRunningAccumulation) 
									intervalHR = totalRemainingAccumulationofHR - log10(numPartitions);
									intervalRunningAccumulation =  -INFINITY;
									}
								}
							#ifdef ABS_SAMPLING_DFS_NON_PROPER_ASSERTIONS
				...
				else if(_Alg.abs_fxn==minVarHRB_dfs){
					//NOTE: currently assumes nodes are from the same variable since DFS
					//do a Knuth abstraction if reached _nLevelsLimit
					AndOrSearchSpace::SearchAndNode *temp_n = (AndOrSearchSpace::SearchAndNode*) OL_to[0] ;
					if (_nLevelsLimit >= 0 && _NumberOfAncestorBranchingVariables[temp_n->V()] >= _nLevelsLimit) {
				...
				else if(_Alg.abs_fxn==randHRB_dfs){
					//NOTE: currently assumes nodes are from the same variable since DFS
					//do a Knuth abstraction if reached _nLevelsLimit
					AndOrSearchSpace::SearchAndNode *temp_n = (AndOrSearchSpace::SearchAndNode*) OL_to[0] ;
					if (_nLevelsLimit >= 0 && _NumberOfAncestorBranchingVariables[temp_n->V()] >= _nLevelsLimit) {
...
////////////////////////
			...
			else if (_CompFn == AbsSamplingTwoAndNodeCompare_QSimple || _CompFn == AbsSamplingTwoAndNodeCompare_RevQSimple){
				if(_Alg.abs_fxn==simpleQB_dfs) {
					//NOTE: currently assumes nodes are from the same variable since DFS
					//do a Knuth abstraction if reached _nLevelsLimit
					AndOrSearchSpace::SearchAndNode *temp_n = (AndOrSearchSpace::SearchAndNode*) OL_to[0] ;
					if (_nLevelsLimit >= 0 && _NumberOfAncestorBranchingVariables[temp_n->V()] >= _nLevelsLimit) {
				...
				else if(_Alg.abs_fxn==balancedQB_dfs){
					//NOTE: currently assumes nodes are from the same variable since DFS
					//do a Knuth abstraction if reached _nLevelsLimit
					AndOrSearchSpace::SearchAndNode *temp_n = (AndOrSearchSpace::SearchAndNode*) OL_to[0] ;
					if (_nLevelsLimit >= 0 && _NumberOfAncestorBranchingVariables[temp_n->V()] >= _nLevelsLimit) {
						...
						double totalAccumlationOfAllQ = -INFINITY;
						if (OL_to.size() > 1) {
							totalAccumlationOfAllQ = QuickSortAccumulated((void**) OL_to.data(), OL_to.size(), left, right, _CompFn, _AccumFn, -INFINITY);
							}
						else{
							totalAccumlationOfAllQ = OL_to[0]->ISq();
							}
						if (totalAccumlationOfAllQ == -INFINITY) {
							SearchAndNode_WithPath *an_merged = DoISmerge(OL_to, 0, OL_to.size()-1) ;
							OL_from.push_back(an_merged) ;
							}
						else{
							double intervalQ = totalAccumlationOfAllQ - log10(_nRandAbs);
							double currentIntervalEnd = intervalQ;
							double currentRunningAccumulation =  -INFINITY;
							int num_abs_states_minus_one = _nRandAbs - 1;
							int32_t idxS = 0, idxE = -1 ;
							while(idxS < OL_to.size()){
								if(nAbsCreated == num_abs_states_minus_one){
									...
									int32_t idx = idxS;
									while(currentRunningAccumulation < currentIntervalEnd){
										LOG_OF_SUM_OF_TWO_NUMBERS_GIVEN_AS_LOGS(currentRunningAccumulation, currentRunningAccumulation, OL_to[idx]->ISq())
										++idx;
										if(idx == OL_to.size()){
											break;
											}
									...
									++nAbsCreated;
									while(currentIntervalEnd <= currentRunningAccumulation){
										LOG_OF_SUM_OF_TWO_NUMBERS_GIVEN_AS_LOGS(currentIntervalEnd, currentIntervalEnd, intervalQ);
										}
									}
								}
							#ifdef ABS_SAMPLING_DFS_NON_PROPER_ASSERTIONS
				...
				else if(_Alg.abs_fxn==balancedQB2_dfs){
					//NOTE: currently assumes nodes are from the same variable since DFS
					//do a Knuth abstraction if reached _nLevelsLimit
					AndOrSearchSpace::SearchAndNode *temp_n = (AndOrSearchSpace::SearchAndNode*) OL_to[0] ;
					if (_nLevelsLimit >= 0 && _NumberOfAncestorBranchingVariables[temp_n->V()] >= _nLevelsLimit) {
				...
				else if(_Alg.abs_fxn==balancedQB3_dfs){
					//NOTE: currently assumes nodes are from the same variable since DFS
					//do a Knuth abstraction if reached _nLevelsLimit
					AndOrSearchSpace::SearchAndNode *temp_n = (AndOrSearchSpace::SearchAndNode*) OL_to[0] ;
					if (_nLevelsLimit >= 0 && _NumberOfAncestorBranchingVariables[temp_n->V()] >= _nLevelsLimit) {
				...
				else if(_Alg.abs_fxn==balancedQB4_dfs){
					//NOTE: currently assumes nodes are from the same variable since DFS
					//do a Knuth abstraction if reached _nLevelsLimit
					AndOrSearchSpace::SearchAndNode *temp_n = (AndOrSearchSpace::SearchAndNode*) OL_to[0] ;
					if (_nLevelsLimit >= 0 && _NumberOfAncestorBranchingVariables[temp_n->V()] >= _nLevelsLimit) {
						...
						if (totalAccumlationOfAllQ == -INFINITY) {
							...
							int numPartitions = _nRandAbs;
							double totalRemainingAccumulationofQ = totalAccumlationOfAllQ;
							double intervalQ = totalRemainingAccumulationofQ - log10(numPartitions);
							double intervalRunningAccumulation =  -INFINITY;
							int num_abs_states_minus_one = _nRandAbs - 1;
							int32_t idxS = 0, idxE = -1 ;
							while(idxS < OL_to.size()){
								if(nAbsCreated == num_abs_states_minus_one){
									...
									int32_t idx = idxS;
									do{
										LOG_OF_SUM_OF_TWO_NUMBERS_GIVEN_AS_LOGS(intervalRunningAccumulation, intervalRunningAccumulation, OL_to[idx]->ISq())
										++idx;
										if(idx == OL_to.size()){
											break;
											}
										} while(intervalRunningAccumulation < intervalQ);
									idxE = idx; //idx already incremented to past old index accessed above
									SearchAndNode_WithPath *an_merged = DoISmerge(OL_to, idxS, idxE) ;
									OL_from.push_back(an_merged) ;
									idxS = idxE ;
									...
									numPartitions -= 1;
									// compute next interval end based on remaining mass and remaining abstract states
									LOG_OF_SUB_OF_TWO_NUMBERS_GIVEN_AS_LOGS(totalRemainingAccumulationofQ, totalRemainingAccumulationofQ, intervalRunningAccumulation) 
									intervalQ = totalRemainingAccumulationofQ - log10(numPartitions);
									intervalRunningAccumulation =  -INFINITY;
									}
								}
							#ifdef ABS_SAMPLING_DFS_NON_PROPER_ASSERTIONS
				...
				else if(_Alg.abs_fxn==minVarQB_dfs){
					//NOTE: currently assumes nodes are from the same variable since DFS
					//do a Knuth abstraction if reached _nLevelsLimit
					AndOrSearchSpace::SearchAndNode *temp_n = (AndOrSearchSpace::SearchAndNode*) OL_to[0] ;
					if (_nLevelsLimit >= 0 && _NumberOfAncestorBranchingVariables[temp_n->V()] >= _nLevelsLimit) {
				...
				else if(_Alg.abs_fxn==randQB_dfs){
					//NOTE: currently assumes nodes are from the same variable since DFS
					//do a Knuth abstraction if reached _nLevelsLimit
					AndOrSearchSpace::SearchAndNode *temp_n = (AndOrSearchSpace::SearchAndNode*) OL_to[0] ;
					if (_nLevelsLimit >= 0 && _NumberOfAncestorBranchingVariables[temp_n->V()] >= _nLevelsLimit) {
			...
			else if (_CompFn == AbsSamplingTwoAndNodeCompare_CustomProper_DFS || _CompFn == AbsSamplingTwoAndNodeCompare_CustomProper){
