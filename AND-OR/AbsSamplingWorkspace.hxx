...
AbsSamplingCompFn AbsSamplingTwoAndNodeCompare_RevHeuristicSimple ;
AbsSamplingCompFn AbsSamplingTwoAndNodeCompare_HRSimple ;
AbsSamplingCompFn AbsSamplingTwoAndNodeCompare_RevHRSimple ;
AbsSamplingCompFn AbsSamplingTwoAndNodeCompare_QSimple ;
AbsSamplingCompFn AbsSamplingTwoAndNodeCompare_RevQSimple ;
...
extern const std::unordered_map<AS_FXNS, AbsSamplingCompFn*> AS_NODE_COMP_FNS ;
extern const std::unordered_map<AS_FXNS, AbsSamplingAccumFn*> AS_ACCUM_FNS ;
extern const std::unordered_map<AS_FXNS, AbsSamplingNodeValueExtractionFn*> AS_NODE_VALUE_FNS ;
extern const std::unordered_map<AS_FXNS, std::array<bool,3>> AS_ALG_PROPERTIES ;
...
struct AS_ALG {
	...
	AbsSamplingNodeValueExtractionFn* node_value_fn = nullptr;
	int nAbs = -1;
	bool AO = true;
	bool proper = false;
	bool dfs = true;
	...
	AS_ALG(AS_FXNS abs_fxn_, 
		...
		:	abs_fxn(abs_fxn_),
			...
			node_value_fn(AS_NODE_VALUE_FNS[abs_fxn_]),
			nAbs(nAbs_),
			AO(AO_),
			proper(proper_),
			dfs(AS_ALG_PROPERTIES[abs_fxn][AS_ALG_PROPERTY_IDX::dfs]),
			randomized(AS_ALG_PROPERTIES[abs_fxn][AS_ALG_PROPERTY_IDX::randomized]),
			dynamic(AS_ALG_PROPERTIES[abs_fxn][AS_ALG_PROPERTY_IDX::dynamic]),
...
protected :
	...
	AbsSamplingNodeValueExtractionFn *_NodeValExtractionFn ;
...
/*		int N = NULL != _Problem ? _Problem->N() : _BFS_expansion_queue_size.size() ;
	...
	AbsSamplingWorkspace(AS_ALG& alg_, unsigned seed_) :
		_Root(this, -1, -1), 
		_Alg(alg_),
		_AS_FN(alg_.abs_fxn),
		_CompFn(alg_.node_comp_fxn), 
		_AccumFn(alg_.accum_fn),
		_NodeValExtractionFn(alg_.node_value_fn), 
