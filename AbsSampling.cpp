...
#define AS_VERSION 3000013 // UAI Version
...
std::string getFileName(const std::string& filePath) {
    ...
    if(lastSeparator == std::string::npos) {
        return filePath; // No separator, return the entire path
    }
...
int main(int argc, char* argv[])
{
	...
	int nParams = argc ;
	unsigned long randomGeneratorSeed = std::chrono::system_clock::now().time_since_epoch().count();
	std::string problem_filepath, problem_filename, evidence_filename, vo_filename, vo_filename_write;
	std::string tree_type = "AO";
	int32_t nrunstodo = -1 ;
	int32_t nArgs = (nParams-1)>>1 ;
	int32_t nContext = INT32_MAX, nAbs = -1 ;
	double time_limit = -1;
	double cvo_time_limit = -1.0;
	int32_t print_period = 1;
	int64_t nLevelsLimit = -1 ;
	...
	int iB = -1 ;
	//Nick edit ------UAI Variables//-------------------------------
	double uaiMemory = 8000000 ; // KB, default just under 8GB
	string uai_output_filename = "" ;
	//Nick edit ----------------------------------------------------
	...
	for (int i = 0 ; i < nArgs; i++) {
		...
		if (0 == stricmp("-s", sArgID.c_str())) 
			...
			iB = atoi(sArg.c_str());
		// Nick edit ---------------------
		else if (0 == stricmp("-uaiMemory", sArgID.c_str())) {
			uaiMemory = atoi(sArg.c_str());
			uaiMemory = uaiMemory / 2; // leave 50% of memory as buffer at first
		}
		else if (0 == stricmp("-uaiOut", sArgID.c_str()))
			uai_output_filename = sArg;
		// Nick edit ---------------------
		...
		// 	forORtreetype_use_DFS_order = '1' == sArg[0] || 'y' == sArg[0] || 'Y' == sArg[0];
			...
			std::cout << "\t" << "The option: '" << sArgID << "' was not recognized." << std::endl;
			std::cout << "Exiting..." << std::endl;
			exit(EXIT_FAILURE);
		}
	}

	// std::mt19937 rng(randomGeneratorSeed);
	
	if (printing) {
		std::cout<<"sanity test2: args are taken \n" << std::flush;
	}

	if (uai_output_filename == ""){
		std::cout<<"UAI output filename not entered" << std::endl << std::flush;
		exit(EXIT_FAILURE);
	}
	...
	if (time_limit > 0.0){
		cvo_time_limit = time_limit;
	}
	else{
		time_limit = 0.0;
	}

	int64_t nLevelsLimitToUse = INT_MAX;
	if (nLevelsLimit > 0){
		nLevelsLimitToUse = nLevelsLimit;
	}
	...
	if (0 == problem_filepath.length()) 
		return 1 ;

	problem_filename = getFileName(problem_filepath);

	if (printing) {
		std::cout<< "sanity test 3\n" << std::flush;
	}

	ARE::ARP p("AbsSamplingProblem") ;
	p.SetOperators(FN_COBINATION_TYPE_PROD, VAR_ELIMINATION_TYPE_SUM) ;
	int resLoad = p.LoadFromFile(problem_filepath) ;
	if (0 != resLoad) 
		return 10 ;
...
//        printf("\nWILL COMPUTE VAR ELIM ORDER...") ;
         ...
         cvoContext._ObjCodePrimary = ARE::VarElimOrderComp::Width ;
         cvoContext._ObjCodeSecondary =
ARE::VarElimOrderComp::StateSpaceSize ;
         cvoContext._nRunsToDoMin = 1 ; //formerly 1000
         cvoContext._nRunsToDoMax = 1000000 ;
         cvoContext._TimeLimitInMilliSeconds = min(300000.0, 500 + cvo_time_limit * 1000 / 20) ; // 300000 = 5min, 60000 = 1 minute,  formerly: 5000 + time_limit * 1000 / 4
         ...
         // 2022-09-20 : for the UAI competition, use just 1 thread...
         cvoContext._nThreads = 1 ; // ARE::maxNumProcessorThreads > 1 ?
// ARE::maxNumProcessorThreads > 1 ? -1 : 1 ;
...
//        printf("\nVAR ELIM ORDER : w=%d", (int) cvoBestOrder._Width) ;
		 ...
		 // BOBAK: ADD VARIABLE ORDER PRINTOUT
		 cvoBestOrder.SerializeAsElimOrder(vo_filename_write.c_str());
         }
     else {
         FILE *fpVO = fopen(vo_filename.c_str(), "rb") ;
         if (NULL == fpVO){
		 	std::cerr << "VO file could not be opened" << std::endl;
             return 40 ;
		 }
         // get file size
         ...
         if (NULL == buf) { 
			fclose(fpVO) ; 
			std::cerr << "Empty VO file" << std::endl;
			return 41 ; 
			}
         ...
         if (filesize != L) { 
			delete [] buf ; 
			std::cerr << "VO file error" << std::endl;
			return 42 ; 
		}
         buf[filesize] = 0 ;
         int32_t OrderType = 0 ;
	...
	if (printing) {
		double wallvo1 = get_wall_time();
		double cpuvo1  = get_cpu_time();

		double wallvo_final = wallvo1 - wall0;
		double cpuvo_final = cpuvo1  - cpu0;
		std::cout<<"Variable ordering done in cpu " << cpuvo_final << " (wall " << wallvo_final <<") sec.\n" << std::flush;
	}
...
//	AndOrSearchSpace::AbsSamplingWorkspace ws(AbsSamplingTwoAndNodeCompare_Unique) ;
	...
	AS_ALG alg(abs_fxn, nAbs, isAO, properAlg, nLevelsLimitToUse, hscale, wscale, wscaleDecayRate, abs_indv);
	AndOrSearchSpace::AbsSamplingWorkspace ws(alg, randomGeneratorSeed) ;
...
//	int resCB = ws.CreateBuckets(true, true, false, isAO) ;
	...
	std::cout << "w*: " << ws.MaxNumVarsInBucket() - 1 << std::endl <<std::flush;
	...
	ws.MaxTreeHeight_BranchingVars_Limit() = nLevelsLimitToUse ;
	int32_t res_MNBV = 
		0 
//		ws.Enfoce_MaxTreeHeight_BranchingVars_Limit() 
	...
	// ws.MaxSpaceAllowed_Log10() = log10(uaiMemory * 1024) ;

	if (printing) {
		std::cout << "Computing i-bound.\n" << std::flush;
	}

	ws.MaxSpaceAllowed_Log10() = ((time_limit > 25) || (time_limit <= 0.0)) ? log10(uaiMemory * 1024) : 6.9 ; //formerly 9.0 for under 20sec
	int iBoundMin = 2 ;
	...
	if (iB <= 0) {
		...
		iB = (time_limit > 25) ? ib - 1 - ((int) ws.Problem()->N() / (time_limit * 10)) : 
			(time_limit <= 0.0) ? ib - 1 : ib - 1 - ((int) ws.Problem()->N() / 20000); //deflate iB to give more as buffer memory (also to speed up compilation)
		}
	else {
		ws.iBound() = iB ;
...
*/
	...
	double cpu_preproc_done = wall_preproc_done ;
	int64_t t_elapsed_preproc_done = wall_preproc_done - wall0 ;
	int64_t t_cpu_elapsed_preproc_done = t_elapsed_preproc_done ;
#else
    double wall_preproc_done = get_wall_time();
	double cpu_preproc_done = get_cpu_time();
	int64_t t_elapsed_preproc_done = (int64_t) (wall_preproc_done - wall0) ;
	int64_t t_cpu_elapsed_preproc_done = (int64_t) (cpu_preproc_done - cpu0) ;

#endif // WINDOWS

	if (printing) {
		std::cout << "\nPreprocessing complete in "<< t_cpu_elapsed_preproc_done << " sec.\n" << std::flush;
	}

	std::string output_filename;
	FILE* output_file;
	// UAI edit
	FILE* uai_output_file;
	// ----------------------------------------------------------------
    ...
    output_filename = (alg.randomized || alg.dynamic) ? 
		(problem_filename + "-" + tree_type + "-i-" + std::to_string(iB) + "-prop-" + (properAlg ? "1" : "0") + "-a-" + algorithm + "-nC-"  + snContext + "-nAbs-"  + snAbs + "-abs_indv-" + sabs_indv + "-hscale-" + std::to_string(int(hscale*100)) + "-wscale-" + std::to_string(int(wscale*100))  + "-wdecay-" + std::to_string(int(wscaleDecayRate*100)) + "-nR-" +  std::to_string(nrunstodo) + "-t-" +  std::to_string((int) time_limit) +".out") 
	    :
		(problem_filename + "-" + tree_type + "-i-" + std::to_string(iB) + "-prop-" + (properAlg ? "1" : "0") + "-a-" + algorithm + "-nC-"  + snContext + "-hscale-" + std::to_string(int(hscale*100)) + "-wscale-" + std::to_string(int(wscale*100))  + "-wdecay-" + std::to_string(int(wscaleDecayRate*100)) + "-nR-" +  std::to_string(nrunstodo) + "-t-" +  std::to_string((int) time_limit) +".out") ;
    output_file = fopen(output_filename.c_str(), "w");
	// UAI edit
	uai_output_file = fopen(uai_output_filename.c_str(), "w");
	// ----------------------------------------------------------------
    fprintf(output_file, "%s \t%d \t%s \t%s \t%d \t%d \t%d \t%d \t%d \t%g \t%g \t%lld \t%c \t%s\n", 
		problem_filepath.c_str(), 
		(int) iB, 
		algorithm.c_str(), 
		snContext.c_str(), 
		(int) nrunstodo, 
		(int) (AS_VERSION), 
		(int) ws.Problem()->N(), 
		(int) (ws.MaxNumVarsInBucket()-1), 
		(int) ws.MaxTreeHeight(), 
		(double) BEvalue_w, 
		(double) exactZ, 
		t_cpu_elapsed_preproc_done, exclude_prep_time ? 'Y' : 'N', 
		tree_type.c_str()) ;
	fflush(output_file);
	// UAI edit
	fflush(uai_output_file);
	// ------------------------------------------------------------------
	if (exclude_prep_time) {
		wall0 = wall_preproc_done;
		cpu0 = cpu_preproc_done ;
	}

	if (printing) {
			std::cout<< "initial write to output files complete.\n" << std::flush;
		}
	...
	printf("%s \t%d \t%d \t%d \t%d \t%d \t%d", problem_filepath.c_str(), (int) p.N(), (int) ws.nBuckets(), (int) (ws.VarOrdering_InducedWidth()), (int) ws.nBucketsWithPartitioning(), (int) ws.Height(), (int) (AS_VERSION)) ;

	if (printing) {
			std::cout<< "\ninitial printf (after initial write to output files) complete.\n" << std::flush;
		}
	...
	if (false) {
		...
		fprintf(fp_log, "problem=%s iB=%d", problem_filepath.c_str(), (int) iB);
	...
	// pseudotree_filename = "Pseudotree/" + problem_filepath + ".pt";
...
// #endif // WINDOWS
	...
	if (printing) {
			std::cout<< "non-Windows printf complete.\n" << std::flush;
		}
	...
	bool uai_first_solution = true ;
	if (nrunstodo <= 0) 
		nrunstodo = std::numeric_limits<int>::max() ;
	if (time_limit > 0){
	...
	for (int32_t i = 0 ; i < nrunstodo ; i++) {
		runsdone++;
		#ifdef LOG_PROBES
			std::cout << std::endl << "Probe: " << runsdone << std::endl;
		#endif //LOG_PROBES
...
//		wall_inter = (wall1 - wall0)/1000.0 ;
        ...
        if (do_print || runsdone == 1 || runsdone == nrunstodo || need_break) { //runsdone%print_period) == 0 || print_period == 1  ||  runsdone == 1
        	...
        	fprintf(output_file, "%d \t%g \t%g \t%lld \t%lld \t%lld \t%lld \t%lld \t%g \t%g \t%lld \t%lld \t%g \t%g\n", 
				(int32_t) i+1, //0
				SAMPLvalue, //1
				(double) (nTriesGood > 0 ? sumOfTries - log10((double)nTriesGood): -std::numeric_limits<float>::infinity()), //2
				(int32_t) nTriesGood, //3
				(int64_t) ws.nNodesInTree(), //4
				(int64_t) ws.nNodesISmerged(), //5
				(int64_t) (sumTreeSize/runsdone), //6
				(int64_t) (nNodesMerged/runsdone), //7
				(double)wall_inter, //8
				(double)cpu_inter, //9
				(int64_t) ws.nNodesCreated(), //10
				(int64_t) (nNodesCreated/runsdone), //11
				(double)variance, //12
				(double)effective_variance); //13
			
			// UAI print statement------------------------------
			if (uai_first_solution) {
				uai_first_solution = false ;
				fprintf(uai_output_file, "PR\n%g\n",
					(double) (nTriesGood > 0 ? sumOfTries - log10((double)nTriesGood): -std::numeric_limits<float>::infinity())) ;
			}
			else {
				fprintf(uai_output_file, "-BEGIN-\n%g\n",
					(double) (nTriesGood > 0 ? sumOfTries - log10((double)nTriesGood): -std::numeric_limits<float>::infinity())) ;
			}
