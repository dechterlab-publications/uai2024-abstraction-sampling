...
int32_t ARE::fileload_getnexttoken(const char * & buf, int32_t & L, const char * & B, int32_t & l, bool IncludeSpecialSymbols)
{
	...
	if (IncludeSpecialSymbols) {
		...
		if (token_is_special_symbol) {
			std::cout<<"special symbol nonsense";
			B = buf ;
			l = 1 ;
			buf += l ;
			L -= l ;
...
// 	int32_t L = fread(BUF, 1, filesize, fp) ;
// 	std::cout << "L is " << L <<"\n";
// 	std::cout << "Re-checking, filesize is " << filesize <<"\n";
// 	fclose(fp) ;
// 	if (filesize != L) {
// 		delete [] BUF ;
...
// 	buf = BUF ;
// 	if (0 != fileload_getnexttoken(buf, L, token, l, false)) {
// 		std::cout << "0 != fileload_getnexttoken(buf, L, token, l, false)" ;
// 		goto done ;
// 	}
// 	int32_t N = atoi(token) ;
// 	if (N < 1) 
...
// 		std::cout << "NULL == var_loaded || NULL == val_loaded" ;
// 		goto done ;
// 	}
...
// 		}
// 		if (val < 0 || val >= _K[var]) {
// 			std::cout << "token is " << token << ", i = " << i << ", val = " << val << ", and _K[var] = " << _K[var] << "\n";
// 			goto done ;
// 		}
// 		var_loaded[n] = var ;
// 		val_loaded[n] = val ;
